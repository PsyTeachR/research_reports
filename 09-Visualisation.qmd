...
# Data Visualisation {#09-Visualisation}

In this chapter, we focus on the role of data visualisation and formatting. We will explore why data visualisation is useful for both you and your reader, but how it is important to consider which types of visualisation transparently and effectively communicate your message. We will also cover some key APA formatting points for presenting tables and figures.
...
To demonstrate why this is important, [the dinosaur dozen](https://www.research.autodesk.com/publications/same-stats-different-graphs/){target="_blank"} shows how the same summary statistics (e.g., mean and standard deviation) can be identical but result from dramatically different data patterns. For example, creating a graph of a star, a cross, and yes, even a dinosaur can have the same underlying mean, standard deviation, and correlation. [Yanai and Lercher (2020)](https://doi.org/10.1186/s13059-020-02133-w){target="_blank"} found that students would not recognise an image of a gorilla recreated as data points when they were focused on rushing to apply a statistical test. Therefore, exploratory data analysis is important in the initial steps of data analysis to check what patterns are present and if the data properties are consistent with what you expect. 
...
Effective (or misleading) data visualisation is a whole field of study, so we will just outline a few key principles to keep in mind when designing your plots to help communicate your data as effectively and honestly as possible. If this is something that interests you, we recommend chapter one - looking at data - from [Data Visualisation](https://socviz.co/index.html#preface){target="_blank"} by Healy (2018) and the comprehensive review article [The Science of Visual Data Communication: What Works](https://doi.org/10.1177/15291006211051956){target="_blank"} by Franconeri et al. (2021) for further reading. 
...
```{r C8 bar, fig.alt = "Demonstrating within the bar bias for continuous outcomes."}
include_graphics("images/08-02-bar.png")
```
...
```{r C8 truncation, fig.alt = "Truncated y-axis demonstration."}
include_graphics("images/08-06-trunc.png")
```
...
```{r C8 colour blind friendly plots, fig.alt = "Differences in colour palettes to help colour blindness."}
include_graphics("images/08-03-colour.png")
```
...
```{r C8 comparison of interest, fig.alt = "Controlling variable order to highlight comparisons of interest."}
include_graphics("images/08-05-comparison.png")
```
...
```{r C8 conceptual message, fig.alt = "Thinking about your conceptual message for how the data should be plotted and honour schemas."}
include_graphics("images/08-04-logicaly.png")
```
...
```{r C8 table, fig.alt = "Demonstration of an APA style table."}
include_graphics("images/08-APA-table.png")
```
...
For full details, the APA style website has a great page on [the key features of an APA formatted table](https://apastyle.apa.org/style-grammar-guidelines/tables-figures/tables){target="_blank"}. 
...
```{r C8 APA figure, fig.alt = "Demonstration of an APA style figure."}
include_graphics("images/08-APA-figure.png")
```
...
For full details, the APA style website has a great page on [the key features of an APA formatted figure](https://apastyle.apa.org/style-grammar-guidelines/tables-figures/figures){target="_blank"}. 
